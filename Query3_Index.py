...
Query3_DBM_Index = dbm.open('q33', 'c')
...
def Thirdquery(blocksDataFeed,blockNumber):
    for i in blocksDataFeed:
        ...
        d = str(blockNumber)
        if search_birthdate in Query3_DBM_Index:
            u = Query3_DBM_Index[search_birthdate]
            i = str(u.decode('utf-8')) + "," + str(blockNumber) + str('-') + str(i[1])
            Query3_DBM_Index[search_birthdate] = i
        else:
            Query3_DBM_Index[search_birthdate] = str(blockNumber) + str('-') + str(i[1])
